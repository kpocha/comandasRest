...
namespace Web.Entidad.Repository
{
    public class Repository<TEntity> : IRepository<TEntity> where TEntity : class, IEntity
    {
        ...
        public virtual TEntity Create(TEntity TEntity)
        {
            try
            {
                if (UtilRepo.HasMember(TEntity, "habilitado"))
                {
                    ...
                    if (UtilRepo.HasMember(TEntity, "fechaAlta"))
                    {
                        Context.Entry(TEntity).Member("fechaAlta").CurrentValue = DateTime.Now;
        ...
        {
            if (UtilRepo.HasMember(TEntity, "fechaModificacion"))
            {
                DbSet.Attach(TEntity);
                Context.Entry(TEntity).Member("fechaModificacion").CurrentValue = DateTime.Now;
        ...
        {
            if (UtilRepo.HasMember(TEntity, "fechaEliminacion"))
            {
                DbSet.Attach(TEntity);
                Context.Entry(TEntity).Member("habilitado").CurrentValue = false;
                Context.Entry(TEntity).Member("fechaEliminacion").CurrentValue = DateTime.Now;
                Context.Entry(TEntity).State = EntityState.Modified;
            }
            else
            {
